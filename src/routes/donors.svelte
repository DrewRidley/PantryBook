	...
	onMount(async () => {
		...
		onSnapshot(collection(db, 'orgs/sfp/clients'), async (e) => {
			...
			e.docs.forEach((doc) => {
				...
				if (doc.id == selectedDonor?.id) {
					selectedDonor = { ...doc.data(), id: doc.id };
				}
			});
			pages = paginate(donors, pagesize);
		});
	});

	async function updateClient() {
		const clientRef = doc(db, 'orgs/sfp/clients', selectedDonor.id);
		await setDoc(clientRef, selectedDonor);
	}
</script>

{#if selectedDonor != null}
	<Modal visible={selectedDonor != null}>
		...
		<div class="mt-4 divide-y divide-gray-200">
			...
			<div class="mt-6">
				<dl class="divide-y divide-gray-200">
					<FormFieldText name="Phone" bind:value={selectedDonor.phone} on:update={updateClient} />
					<FormFieldText name="Email" bind:value={selectedDonor.email} on:update={updateClient} />
					<FormFieldSelect name='Veteran' bind:value={selectedDonor.vet} on:update={updateClient} options={['Yes', 'No']}/>
					<FormFieldSelect name='Household Revenue' bind:value={selectedDonor.revenue} on:update={updateClient} options={['<20k/yr', '20-40k/yr', '40-60k/yr', '60-80k/yr', '80+k/yr']}/>
				</dl>
			</div>
		</div>
	</Modal>
											...
											<div class="px-8 py-6 mx-4 flex-auto">
												...
												<div class="mt-1 relative rounded-md shadow-sm">
													...
													<input
														...
														on:input={() => {
															let newArr = [];
															for (let x = 0; x < donors.length; x++) {
																if (
																	donors[x].name.toLowerCase().includes(searchTerm.toLowerCase())
																) {
																	newArr.push(donors[x]);
																}
															}
